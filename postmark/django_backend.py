...
class EmailBackend(BaseEmailBackend):
    ...
    def __init__(self, api_key=None, default_sender=None, **kwargs):
        ...
        self.api_key = getattr(settings, 'EMAILMANAGER_API_KEY', api_key)
        if not self.api_key:
            raise ImproperlyConfigured('EMAILMANAGER API key must be set in Django settings file or passed to backend constructor.')            
        self.default_sender = getattr(settings, 'EMAILMANAGER_SENDER', default_sender)
        self.test_mode = getattr(settings, 'EMAILMANAGER_TEST_MODE', False) 
    ...
    def _send(self, message):
        ...
        try:
            ...
            emailmanager_message = PMMail(api_key=self.api_key, 
            ...
            emailmanager_message.tag = getattr(message, 'tag', None)
            
            emailmanager_message.send(test=self.test_mode)
        except:
