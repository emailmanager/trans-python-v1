...
try:
    ...
    try:
        ...
        raise Exception('Cannot use python-emailmanager library without Python 2.6 or greater, or Python 2.4 or 2.5 and the "simplejson" library')

#
#
__EMAILMANAGER_URL__ = 'http://trans.emailmanager.com/'

class PMMail(object):
    '''
    The Emailmanager Mail object.
    '''
    def __init__(self, **kwargs):
        '''
        Keyword arguments are:
        api_key:        Your Emailmanager server API key
        ...
        self.__user_agent = 'Python/%s (python-emailmanager library version %s)' % ('_'.join([str(var) for var in sys.version_info]), __version__)
        ...
        try:
            ...
            if not self.__api_key and hasattr(django_settings, 'EMAILMANAGER_API_KEY'):
                self.__api_key = django_settings.EMAILMANAGER_API_KEY
            if not self.__sender and hasattr(django_settings, 'EMAILMANAGER_SENDER'):
                self.__sender = django_settings.EMAILMANAGER_SENDER
        except ImportError:
            pass
    ...
    api_key = property(
        ...
        '''
        The API Key for your rack server on Emailmanager
        '''
    )
    ...
    sender = property(
        ...
        The address should match one of your Sender Signatures in Emailmanager.
    ...
    reply_to = property(
        ...
        format. The reply-to address does not have to be one of your Sender Signatures in Emailmanager.
    ...
    tag = property(
        ...
        you will be able to get detailed statistics for them through the Emailmanager user interface.
        '''
    )
...
#         'The API Key for one of your servers on Emailmanager'
#     )
    ...
    def _check_values(self):
        ...
        if not self.__api_key:
            raise PMMailMissingValueException('Cannot send an e-mail without a Emailmanager API Key')
    ...
    def send(self, test=None):
        '''
        Send the email through the Emailmanager system.  
        Pass test=True to just print out the resulting
        JSON message being sent to Emailmanager
        '''
        self._check_values()
...
#             pass
        ...
        if test is None:
            try:
                from django.conf import settings as django_settings
                test = getattr(django_settings, "EMAILMANAGER_TEST_MODE", None)
            except ImportError:
                pass
        ...
        req = urllib2.Request(
            __EMAILMANAGER_URL__ + 'email',
            json.dumps(json_message),
            {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                'X-Emailmanager-Server-Token': self.__api_key,
        ...
        # Attempt send
        try:
            #print 'sending request to emailmanager: %s' % json_message
            ...
            if err.code == 401:
                ...
                raise PMMailServerErrorException('Internal server error at Emailmanager. Admins have been alerted.', err)
...
class PMBatchMail(object):
    ...
    def send(self, test=None):
        ...
        req = urllib2.Request(
            __EMAILMANAGER_URL__ + 'email/batch',
            json.dumps(json_message),
            {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
...
class PMBounceManager(object):
    '''
    The Emailmanager Bounce object.
    '''
    def __init__(self, **kwargs):
        '''
        Keyword arguments are:
    ...
    def _check_values(self):
        ...
        '''
        if not self.__api_key:
            raise PMMailMissingValueException('Cannot check bounces without a Emailmanager Server API Key')      
    ...
    def delivery_stats(self):
        ...
        req = urllib2.Request(
            __EMAILMANAGER_URL__ + 'deliverystats',
            None,
            {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
        ...
        # Attempt send
        try:
            #print 'sending request to emailmanager:'
            result = urllib2.urlopen(req)
            if result.code == 200:
    ...
    def get_all(self, inactive='', email_filter='', tag='', count=25, offset=0):
        ...
        req = urllib2.Request(  	
            __EMAILMANAGER_URL__ + 'bounces' + params,
            None,
            {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    def get_single(self, bounce_id):
        ...
        req = urllib2.Request(
            __EMAILMANAGER_URL__ + '/bounces/' + str(bounce_id),
            None,
            {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    def get_dump(self, bounce_id):
        '''
    	Returns the raw source of the bounce Emailmanager accepted. If Emailmanager does not have a dump for 
        that bounce, it will return an empty string.
    	'''
        self._check_values()

        req_url = __EMAILMANAGER_URL__ + '/bounces/' + str(bounce_id) + '/dump'
        #print req_url
    ...
    def get_tags(self):
        ...
        req = urllib2.Request(
            __EMAILMANAGER_URL__ + 'bounces/tags',
            None,
            {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
    ...
    def activate(self, bounce_id):
        ...
        #print req_url
        h1 = httplib.HTTPConnection('trans.emailmanager.com')
